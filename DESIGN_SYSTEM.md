...
Prefer **self-generated** assets over third-party services. The public
instances below are free, unfunded, and go down without warning — anything
critical to the profile should be produced by a workflow in this repo.

| Source | Status | Notes |
|--------|--------|-------|
| Snake graph (`Platane/snk`) | Self-hosted | Generated by `snake.yml`, published to the `output` branch |
| Metrics (`lowlighter/metrics`) | Self-hosted | Generated by `metrics.yml` into `assets/metrics.svg`; needs `METRICS_TOKEN` |
| `streak-stats.demolab.com` | Live | Current home of streak stats |
| `github-readme-activity-graph.vercel.app` | Live | Activity area chart |
| `img.shields.io` | Live | Follower/star badges |
| `github-readme-stats.vercel.app` | **Down (503)** | Public instance over quota — do not use unless self-hosted |
| `github-profile-trophy.vercel.app` | **Down (402)** | Vercel quota exceeded |
| `github-readme-streak-stats.herokuapp.com` | Deprecated | Redirects; use `streak-stats.demolab.com` |

### GitHub Markdown Constraints

GitHub sanitizes README HTML. This constrains the design system directly:

- `style` attributes are **stripped** — no inline CSS, no glassmorphism, no gradients, no progress bars built from `<div>`s
- `<script>`, `<style>` blocks, and class-based styling are removed
- Allowed: `<div align>`, `<p align>`, `<img>`, `<picture>`, `<table>`, `<details>`, `<sub>`, `<br>`
- Anything visual beyond that must be baked into an **SVG asset**, where the full design system applies

In practice: the tokens below govern the SVGs in `assets/`, not the README body.
...
├── assets/
│   ├── banner/
│   │   └── header-dark.svg          # rendered in README
│   └── svg/
│       ├── icons/                   # rendered in README
│       │   ├── python.svg
│       │   ├── fastapi.svg
│       │   ├── langchain.svg
│       │   ├── langgraph.svg
...
│       │   ├── openai.svg
│       │   └── gemini.svg
│       ├── hero-neural-network.svg  # unused — kept for future layouts
│       ├── particles-bg.svg         # unused
│       ├── neural-lines.svg         # unused
│       ├── connecting-lines.svg     # unused
│       ├── glow-gradient.svg        # unused
│       ├── status-pulse.svg         # unused
│       └── glass-sphere.svg         # unused
├── DESIGN_SYSTEM.md
└── README.md
```

> `assets/metrics.svg` is generated by CI and is not checked in by hand.
